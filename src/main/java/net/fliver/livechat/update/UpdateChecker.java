...
public final class UpdateChecker implements Listener {
  ...
  private static final String FLIVER_DOWNLOAD = "https://fliver.net/api/live-chat/download";
  private static final String FALLBACK_TEMPLATE =
      "&8[&dLive Chat&8] &r&eNew version &f%latest% &7(you have &f%current%&7). &aClick to download";
  ...
  public void onJoin(PlayerJoinEvent event) {
    ...
    Component message = formatUpdateComponent(update.current(), update.latest());
    plugin
        .trio()
        .scheduler()
        .later(
  ...
  private Release fetchLatest() throws IOException, InterruptedException {
    ...
    String jarUrl = null;
    for (Object raw : Json.asArray(body.getOrDefault("assets", List.of()))) {
      Map<String, Object> asset = Json.asObject(raw);
      String name = Json.asString(asset.get("name"), "");
      if (name.endsWith(".jar")) {
        jarUrl = Json.asString(asset.get("browser_download_url"), null);
        break;
      }
    }

    String downloadUrl =
        jarUrl != null && !jarUrl.isBlank()
            ? jarUrl
            : (htmlUrl != null && !htmlUrl.isBlank() ? htmlUrl : FLIVER_DOWNLOAD);

    return new Release(tag, htmlUrl, downloadUrl);
  }

  private void broadcast(PendingUpdate update) {
    Component playerMessage = formatUpdateComponent(update.current(), update.latest());
    String consoleMessage = formatConsoleMessage(update.current(), update.latest());
    plugin
        .trio()
        .scheduler()
        .sync(
  ...
  private Component formatUpdateComponent(String current, Release latest) {
    String colored;
    if (plugin.trio().lang() != null && plugin.trio().lang().has("update.available")) {
      ...
      colored =
          FALLBACK_TEMPLATE
              ...
              .replace("%url%", latest.downloadUrl());
    }

    Component body = LegacyComponentSerializer.legacyAmpersand().deserialize(colored);
    return body
        .clickEvent(ClickEvent.openUrl(latest.downloadUrl()))
        .hoverEvent(
            HoverEvent.showText(
                Component.text("Click to download", NamedTextColor.GRAY)
                    .append(Component.newline())
                    .append(Component.text(latest.htmlUrl(), NamedTextColor.DARK_GRAY))));
  }

  private String formatConsoleMessage(String current, Release latest) {
    if (plugin.trio().lang() != null && plugin.trio().lang().has("update.available")) {
      return plugin
          .trio()
          .messages()
  ...
  private record Release(String tag, String htmlUrl, String downloadUrl) {}
